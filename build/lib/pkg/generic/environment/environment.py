...
def GetENV(experimentDetails):
	experimentDetails.ExperimentName =  os.getenv("EXPERIMENT_NAME", "chaos")
	...
	experimentDetails.ChaosInjectCmd = os.getenv("CHAOS_INJECT_COMMAND", "sleep 400 &")
	experimentDetails.ChaosKillCmd = os.getenv("CHAOS_KILL_COMMAND", "kill $(find /proc -name exe -lname '*/sleep' 2>&1 | grep -v 'Permission denied' | awk -F/ '{print $(NF-1)}')")
